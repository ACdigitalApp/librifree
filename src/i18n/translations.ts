export type Locale = "it" | "en";

export const translations = {
  it: {
    heroTitle: "Librifree – Biblioteca Digitale Gratuita",
    heroSubtitle: "Leggi gratuitamente migliaia di libri classici online.",
    enterLibrary: "Esplora il Catalogo",
    library: "Biblioteca",
    librarySubtitle: "Scegli un libro e inizia a leggere.",
    home: "Home",
    coverAlt: "Copertina di",
    searchPlaceholder: "Cerca per titolo o autore...",
    allCategories: "Tutte le categorie",
    loadMore: "Carica altri libri",
    noResults: "Nessun libro trovato.",
    bookSingular: "libro",
    bookPlural: "libri",
    bookNotFound: "Libro non trovato",
    backToLibrary: "Torna alla Biblioteca",
    backToBook: "Torna al libro",
    tableOfContents: "Indice",
    italiano: "Italiano",
    english: "English",
    summaryOf: "Riassunto di",
    aiSummary: "Riassunto generato con AI",
    aiGenerated: "Contenuto generato con AI",
    noSummaryYet: "Nessun riassunto disponibile per questo libro.",
    noContentYet: "Contenuto non ancora disponibile per questo libro.",
    generateSummary: "Genera riassunto con AI",
    generateContent: "Genera contenuto con AI",
    generating: "Generazione in corso...",
    readFullBook: "Leggi il libro completo",
    readSummary: "Leggi il riassunto",
    by: "di",
    download: "Scarica",
    source: "Fonte",
    popularBooks: "Libri popolari",
    recentBooks: "Aggiunti di recente",
    mainCategories: "Categorie principali",
    viewAll: "Vedi tutti",
    booksCount: "libri",
    exploreCategory: "Esplora",
    charactersOf: "Personaggi di",
    quotesFrom: "Citazioni da",
    analysisOf: "Analisi di",
    chaptersOf: "Capitoli di",
    characters: "Personaggi",
    quotes: "Citazioni",
    analysis: "Analisi",
    chapters: "Capitoli",
    themes: "Temi",
    themesOf: "Temi di",
    historicalContext: "Contesto storico",
    historicalContextOf: "Contesto storico di",
    authorBooks: "Libri dell'autore",
    exploreMore: "Esplora di più",
    buyPhysicalBook: "Compra il libro cartaceo",
    buyPhysicalBookDesc: "Supporta gli autori acquistando la versione cartacea.",
    buyOnAmazon: "Cerca su Amazon",
    recommendedBooks: "Libri consigliati",
    eduSection: "Sezione Scuola",
    eduOptimized: "Ottimizzato per ricerche scolastiche.",
    schoolSummary: "Riassunto per la scuola",
    schoolAnalysis: "Analisi scolastica",
    charactersExplained: "Personaggi spiegati",
    schoolSummaryOf: "Riassunto per la scuola di",
    schoolAnalysisOf: "Analisi scolastica di",
    charactersExplainedOf: "Personaggi spiegati di",
  },
  en: {
    heroTitle: "Librifree – Free Digital Library",
    heroSubtitle: "Read thousands of classic books online for free.",
    enterLibrary: "Explore Catalog",
    library: "Library",
    librarySubtitle: "Choose a book and start reading.",
    home: "Home",
    coverAlt: "Cover of",
    searchPlaceholder: "Search by title or author...",
    allCategories: "All categories",
    loadMore: "Load more books",
    noResults: "No books found.",
    bookSingular: "book",
    bookPlural: "books",
    bookNotFound: "Book not found",
    backToLibrary: "Back to Library",
    backToBook: "Back to book",
    tableOfContents: "Table of Contents",
    italiano: "Italiano",
    english: "English",
    summaryOf: "Summary of",
    aiSummary: "AI-generated summary",
    aiGenerated: "AI-generated content",
    noSummaryYet: "No summary available for this book yet.",
    noContentYet: "Content not available for this book yet.",
    generateSummary: "Generate AI summary",
    generateContent: "Generate AI content",
    generating: "Generating...",
    readFullBook: "Read the full book",
    readSummary: "Read summary",
    by: "by",
    download: "Download",
    source: "Source",
    popularBooks: "Popular books",
    recentBooks: "Recently added",
    mainCategories: "Main categories",
    viewAll: "View all",
    booksCount: "books",
    exploreCategory: "Explore",
    charactersOf: "Characters of",
    quotesFrom: "Quotes from",
    analysisOf: "Analysis of",
    chaptersOf: "Chapters of",
    characters: "Characters",
    quotes: "Quotes",
    analysis: "Analysis",
    chapters: "Chapters",
    themes: "Themes",
    themesOf: "Themes of",
    historicalContext: "Historical context",
    historicalContextOf: "Historical context of",
    authorBooks: "Books by author",
    exploreMore: "Explore more",
    buyPhysicalBook: "Buy the physical book",
    buyPhysicalBookDesc: "Support authors by purchasing the print edition.",
    buyOnAmazon: "Search on Amazon",
    recommendedBooks: "Recommended books",
  },
} as const;

export type TranslationKey = keyof typeof translations.it;
